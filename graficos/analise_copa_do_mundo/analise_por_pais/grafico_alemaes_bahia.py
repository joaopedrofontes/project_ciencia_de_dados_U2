# Gráfico comparando as chegadas de Alemães por mês, na Bahia, durante o período da copa do mundo
...
df_alemanha_bahia = df[(df['uf'] == 'Bahia') &
                       (df['ano'].between(2013, 2015)) &
                       (df['país'] == 'Alemanha')]

chegadas_alemanha_bahia_mensal = df_alemanha_bahia.groupby(['ano', 'cod mes'])['chegadas'].sum().reset_index()

chegadas_alemanha_bahia_mensal['data'] = pd.to_datetime(chegadas_alemanha_bahia_mensal['ano'].astype(str) + '-' + chegadas_alemanha_bahia_mensal['cod mes'].astype(str) + '-01')
...
plt.plot(df_final['data'], df_final['chegadas'], marker='o', color='black')
plt.title('Chegadas de Alemães na Bahia (2013–2015)', fontsize=14)
